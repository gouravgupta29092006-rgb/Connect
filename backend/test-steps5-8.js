...
async function runTests() {
  ...
  // Fetch latest project ID (created in step 4 tests)
  ...
  const roadmapBody = JSON.stringify({
    ...
    description: 'A mobile app using ML to help students navigate campus, find classrooms, and avoid crowded areas.',
    skills: ['React Native', 'Python', 'Machine Learning', 'PostgreSQL'],
    teamSize: 4,
    duration: '4 months',
  });
  ...
  if (t4.body.roadmap?.phases) {
    ...
    console.log(`     → AI: ${t4.body.ai_available}, phases: ${t4.body.roadmap.phases?.length || 'N/A'}`);
  } else {
    console.log(`     → AI: ${t4.body.ai_available} (fallback roadmap used)`);
  }
  ...
  console.log(`     → AI: ${t6.body.ai_available}`);
  ...
  console.log('\n💡  To enable AI summaries: add GEMINI_API_KEY to .env');
