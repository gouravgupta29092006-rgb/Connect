...
async function runTests() {
  ...
  // ── Setup: Register a second user to apply, log both in ──────────────────
  const reg2Body = JSON.stringify({ email: 'applicant@connect.dev', password: 'password123', full_name: 'Applicant User', institution: 'NIT Trichy' });
  ...
  // Login as owner (autotest)
  const loginOwner = JSON.stringify({ email: 'autotest@connect.dev', password: 'password123' });
  ...
  // Login as applicant
  const loginApp = JSON.stringify({ email: 'applicant@connect.dev', password: 'password123' });
